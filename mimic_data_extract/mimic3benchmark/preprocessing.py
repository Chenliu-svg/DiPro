...
def assemble_episodic_data(stays, diagnoses):
    ...
    # merge operation should be performed based on the indexes of both DataFrames.
...
def extract_diagnosis_labels(diagnoses):
    ...
    # labels = pd.concat([diagnoses[['stay_id', 'icd_code', 'value']].drop_duplicates() \
    #                    .pivot(index='stay_id', columns='icd_code', values='value').fillna(0).astype(int)], axis=1)

    # f the label is not present as a column in the labels DataFrame,
    # a new column with that label is added and filled with 0.
...
def read_hw_itemid_to_variable_map(fn, variable_column='LEVEL2'):
    ...
    # only get height and weight
    # var_map = var_map[var_map.variable.isin(['Height', 'Weight'])]
...
def read_itemid_to_variable_map(fn, variable_column='LEVEL2'):
    var_map = pd.read_csv(fn).fillna('').astype(str)
    var_map.COUNT = var_map.COUNT.astype(int)
    # var_map=var_map[var_map.COUNT>10000]
    var_map.ITEMID = var_map.ITEMID.astype(int)
    var_map = var_map[(var_map[variable_column] != '') & (var_map.COUNT > 10000) & (var_map.ITEMID>220000)]
    # var_map = var_map[(var_map.STATUS == 'ready')]

    var_map = var_map[[variable_column, 'ITEMID', 'MIMIC LABEL', 'CATEGORY','PARAM_TYPE','COUNT']]
    # .set_index('ITEMID')
    var_map = var_map.rename({variable_column: 'variable', 'MIMIC LABEL': 'mimic_label'}, axis=1)
    ...
    # if varaible name is the smae then choose the one with the largest count
    ...
    var_map_sorted=var_map.sort_values(['variable','count'],ascending=True)
    var_map_most=var_map_sorted.groupby(['variable']).nth(-1).reset_index()
        # import pdb; pdb.set_trace()
